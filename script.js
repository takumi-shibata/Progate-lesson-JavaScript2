...
for (let i = 0; i < characters.length; i += 1) {　//name・ageそれぞれ出力するのは手間がかかるので繰り返し処理。
  ...
  const character = characters[i]; //charactersの代入内容とfor文内の内容[i]をcharacterに代入
...
for (let i = 0; i < characters.length; i += 1) {
  ...
  // if文を追加してください
  if (character.age === undefined) {　//if文でundefinedの時の処理。「===」 →　a(character.age)が、b(undefined)になる時は(と一致する時は)
    console.log("年齢は秘密です");
  } else {　//if文に該当しない時の処理(character.ageの値が定義されている時の処理)
